...
window.User = {
  ...
  */
  ...
  SIGNIN_BUTTONS:['BTN_SIGNIN', 'BTN_SIGNUP'],
  prepare_signin_form: function(){
    ...
    for( i in this.SIGNIN_LABELS ){
      ...
      replace   = LOCALE_UI.User.Signin[label_id] ;
      $('div#user_signin_form td#'+td_id).html( replace ) ;
    }
    // Les boutons
    for(i in this.SIGNIN_BUTTONS){
      label_id  = this.SIGNIN_BUTTONS[i] ;
      td_id     = label_id.toLowerCase();
      replace   = LOCALE_UI.User.Signin[label_id] ;
      $('div#user_signin_form a#'+td_id).text( replace ) ;
    }
  ...
  */
  ...
  check_data:function(){
    try{
      ...
      if ( pwd == "" || pwd != pwd.replace(/[^a-z0-9]/g, '')) throw 'bad_password' ;
  ...
  prepare_signup_form: function(){
    ...
    // Les boutons
    for(i in this.SIGNUP_BUTTONS){
      label_id  = this.SIGNUP_BUTTONS[i] ;
      search    = 'SIGNUP_' + label_id ;
      td_id     = search.toLowerCase();
      replace   = LOCALE_UI.User.Signup[label_id] ;
      $('div#user_signup_form a#'+td_id).text( replace ) ;
    }
    this.preparing_form = false ;
  },
